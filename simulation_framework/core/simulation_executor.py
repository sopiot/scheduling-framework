...
class MXSimulatorExecutor:
    ...
    def __init__(self, simulation_file_path: str = None, mqtt_debug: bool = False, middleware_debug: bool = False, args: dict = None) -> None:
        ...
        self.simulation_event_timeline: List[MXEvent] = []
    ...
    def load_simulation(self, simulation_folder_path: str):
        ...
        self.simulation_env = MXMiddlewareElement().load(simulation_data['component'])

        self.event_handler = MXEventHandler(simulation_env=self.simulation_env,
        ...
        self.simulation_event_timeline = [MXEvent(event_type=MXEventType.get(event['event_type']),
                                                  element=find_element_by_name_recursive(self.simulation_env, event['element'])[0],
    ...
    def build_middleware_thing_env(self):
        simulation_env_build_timeline = [event for event in self.simulation_event_timeline if event.event_type
                                         in [MXEventType.MIDDLEWARE_RUN, MXEventType.THING_RUN, MXEventType.THING_REGISTER_WAIT, MXEventType.DELAY]]
        last_index = [event.event_type for event in simulation_env_build_timeline].index(MXEventType.DELAY)
    ...
    def build_scenario_env(self):
        ...
        def task(middleware_scenario_add_timeline: List[MXEvent]):
            for event in middleware_scenario_add_timeline:
                self.event_handler.event_trigger(event)

        whole_scenario_add_timeline = [event for event in self.simulation_event_timeline if event.event_type
                                       in [MXEventType.SCENARIO_ADD, MXEventType.SCENARIO_ADD_CHECK, MXEventType.REFRESH, MXEventType.DELAY]][1:]
        ...
        for middleware in middleware_list:
            scenario_add_timeline = [event for event in [event for event in whole_scenario_add_timeline
                                     if event.event_type == MXEventType.SCENARIO_ADD]
                                     if find_element_recursive(self.simulation_env, event.element)[1].name == middleware.name]
            middleware_scenario_add_timeline_list.append(scenario_add_timeline)

        scenario_check_timeline_list = [event for event in whole_scenario_add_timeline if event.event_type in [MXEventType.SCENARIO_ADD_CHECK, MXEventType.REFRESH, MXEventType.DELAY]]
    ...
    def simulation_event_trigger_start(self):
        start_index = [event.event_type for event in self.simulation_event_timeline].index(MXEventType.START)
        end_index = [event.event_type for event in self.simulation_event_timeline].index(MXEventType.END)
    ...
    def generate_middleware_file(self, simulation_env: MXMiddlewareElement):
        ...
        for middleware in middleware_list:
            ...
            middleware.middleware_cfg_file(self.simulation_env, remote_home_dir)
    ...
    def generate_thing_file(self, simulation_env: MXMiddlewareElement):
    ...
    def generate_scenario_file(self, simulation_env: MXMiddlewareElement):
        scenario_list: List[MXScenarioElement] = get_scenario_list_recursive(simulation_env)
        for scenario in scenario_list:
            write_file(scenario.scenario_file_path, scenario.scenario_code())

    def send_middleware_file(self, simulation_env: MXMiddlewareElement):

        def ssh_task(middleware: MXMiddlewareElement):
            ssh_client = self.event_handler.find_ssh_client(middleware)
        ...
        def send_task(middleware: MXMiddlewareElement):
            ssh_client = self.event_handler.find_ssh_client(middleware)
            user = middleware.device.user
            ssh_client.send_command(f'mkdir -p {home_dir_append(middleware.remote_middleware_config_path, user)}')
    ...
    def send_thing_file(self, simulation_env: MXMiddlewareElement):

        def ssh_task(thing: MXThingElement):
        ...
        def send_task(thing: MXThingElement):
            ssh_client = self.event_handler.find_ssh_client(thing)
            user = thing.device.user
            ssh_client.send_command(f'mkdir -p {home_dir_append(os.path.dirname(thing.remote_thing_file_path), user)}')
            try:
                ...
                os.system(f'sshpass -p "{ssh_client.device.password}" scp -o StrictHostKeyChecking=no -P {ssh_client.device.ssh_port} \
                            {os.path.abspath(thing.thing_file_path)} {ssh_client.device.user}@{ssh_client.device.host}:{thing.remote_thing_file_path} \
                            > /dev/null 2>&1 &')

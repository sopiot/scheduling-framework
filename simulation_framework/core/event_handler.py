...
class MXEventHandler:
    ...
    def __init__(self, simulation_env: MXMiddlewareElement = None, event_log: List[MXEvent] = [], timeout: float = 5.0, mqtt_debug: bool = False, middleware_debug: bool = False, running_time: float = None,
        ...
        self.middleware_list: List[MXMiddlewareElement] = get_middleware_list_recursive(
            self.simulation_env)
        self.thing_list: List[MXThingElement] = get_thing_list_recursive(
            self.simulation_env)
        self.scenario_list: List[MXScenarioElement] = get_scenario_list_recursive(
            self.simulation_env)
        ...
        self.event_listener_thread: MXThread = MXThread(
    ...
    def update_middleware_thing_device_list(self):
        device_list: List[MXDeviceElement] = [
        ...
        def task(device: MXDeviceElement):
            ...
            if device.mqtt_port:
                ...
                if '192.168' in device.host or device.host == 'localhost':
                    ...
                    MXTEST_LOG_DEBUG(
                        f'Found available port of {device.name}', MXTestLogLevel.INFO)
                else:
                    raise Exception(
            ...
            if event.event_type == MXEventType.MIDDLEWARE_RUN:
                ...
                if parent_middleware:
                    while not parent_middleware.online:
                        time.sleep(THREAD_TIME_OUT)
                MXThread(name=f'{event.event_type.value}_{event.element.name}',
                ...
                raise MXTEST_LOG_DEBUG(
                    f'Event type is {event.event_type}, but not implemented yet', MXTestLogLevel.FAIL)
    ...
    def run_middleware(self, middleware: MXMiddlewareElement, timeout: float = 5):
        ...
        def check_online(mqtt_client: MXMQTTClient, timeout: float):
            _, payload, _ = self.publish_and_expect(
                middleware,
                encode_MQTT_message(MXProtocolType.WebClient.EM_REFRESH.value %
            ...
            if payload is not None:
                MXTEST_LOG_DEBUG(
                    f'Middleware {middleware.name} on {middleware.device.host}:{middleware.mqtt_port} - websocket: {middleware.websocket_port} was online!', MXTestLogLevel.PASS)
    ...
    def check_thing_register(self, thing: MXThingElement, timeout: float = 5):
        ...
        if payload is None:
            MXTEST_LOG_DEBUG(
                f'TM_REGISTER of thing {thing.name} was not detected (timeout)...', MXTestLogLevel.FAIL)
            return False
        ...
        if payload is None:
            MXTEST_LOG_DEBUG(
                f'MT_RESULT_REGISTER thing {thing.name} was not detected (timeout)...', MXTestLogLevel.FAIL)
            return False
        ...
        if error in [0, -4]:
            ...
            MXTEST_LOG_DEBUG(
                f'Register fail of thing {thing.name} with register packet error... error code: {payload["error"]}', MXTestLogLevel.FAIL)
            return False
    ...
    def subscribe_thing_topic(self, thing: MXThingElement, mqtt_client: MXMQTTClient):
        for service in thing.service_list:
            mqtt_client.subscribe([MXProtocolType.Base.MT_EXECUTE.value % (service.name, thing.name, thing.middleware_client_name, '#'),
                                   (MXProtocolType.Base.MT_EXECUTE.value % (
                                       service.name, thing.name, '', '')).rstrip('/'),
                                   MXProtocolType.Base.TM_RESULT_EXECUTE.value % (
            ...
            if thing.is_super:
                mqtt_client.subscribe([
                    ...
                    MXProtocolType.Super.MS_RESULT_EXECUTE.value % (
                        '+', '+', '#'),
                    MXProtocolType.Super.SM_RESULT_EXECUTE.value % (
                        service.name, thing.name, thing.middleware_client_name, '#'),
                    MXProtocolType.Super.MS_SCHEDULE.value % (
                        service.name, thing.name, thing.middleware_client_name, '#'),
                    MXProtocolType.Super.SM_SCHEDULE.value % (
                        '+', '+', '#'),
                    MXProtocolType.Super.MS_RESULT_SCHEDULE.value % (
                        '+', '+', '#'),
                    MXProtocolType.Super.SM_RESULT_SCHEDULE.value % (
                        service.name, thing.name, thing.middleware_client_name, '#')])
    ...
    def run_thing(self, thing: MXThingElement, timeout: float = 5):
        # MXTEST_LOG_DEBUG(
        #     f'Thing {thing.name} run start...', MXTestLogLevel.PASS)
        ...
        if self.check_thing_register(thing, timeout=timeout):
            # MXTEST_LOG_DEBUG(
            #     f' Thing Register is complete. Thing: {thing.name}, Middleware: {middelware.name}', MXTestLogLevel.PASS)
    ...
    def unregister_thing(self, thing: MXThingElement):
        MXTEST_LOG_DEBUG(
            f'Unregister Thing {thing.name}...', MXTestLogLevel.INFO, color='yellow')
        ssh_client = self.find_ssh_client(thing)
        ssh_client.send_command(f'kill -2 {thing.pid}')

    def kill_thing(self, thing: MXThingElement):
        MXTEST_LOG_DEBUG(
            f'Kill Thing {thing.name}...', MXTestLogLevel.INFO, color='yellow')
        ssh_client = self.find_ssh_client(thing)
    ...
    def refresh(self, timeout: float, service_check: bool = False, scenario_check: bool = False):
        ...
        def task(middleware: MXMiddlewareElement):
            MXTEST_LOG_DEBUG(
                f'Refresh Start... middleware: {middleware.name}', MXTestLogLevel.INFO)
            ...
            if service_check:
                ...
                if whole_service_info is False:
                    return False
            if scenario_check:
                whole_scenario_info_list: List[MXScenarioInfo] = self.get_whole_scenario_info(
                    middleware, timeout=timeout)
            ...
            for scenario in middleware.scenario_list:
                ...
                if set(target_scenario_service_name_list).issubset(set(whole_service_name_info)):
                    ...
                    MXTEST_LOG_DEBUG(
                        f'Service check Fail... level: {middleware.level}, middleware: {middleware.name}, scenario: {scenario.name}', MXTestLogLevel.WARN)
            ...
            for scenario in middleware.scenario_list:
                for scenario_info in whole_scenario_info_list:
                    ...
                    MXTEST_LOG_DEBUG(
                        f'Scenario {scenario.name} is not in scenario list of {middleware.name}...', MXTestLogLevel.WARN)

            MXTEST_LOG_DEBUG(
                f'Refresh Success! middleware: {middleware.name}', MXTestLogLevel.INFO)
    ...
    def thing_register_wait(self):
        while not all([thing.registered for thing in self.thing_list]):
            time.sleep(THREAD_TIME_OUT)
        MXTEST_LOG_DEBUG(
            f'All thing register is complete!...', MXTestLogLevel.INFO)
    ...
    def scenario_add_check(self, timeout: float):
        while not all([scenario.schedule_success for scenario in self.scenario_list]):
            time.sleep(THREAD_TIME_OUT)
        MXTEST_LOG_DEBUG(
            f'All scenario Add is complete!...', MXTestLogLevel.INFO)
                ...
                for scenario_info in whole_scenario_info_list:
                    ...
                    if scenario.name == scenario_info.name:
                        if scenario.state in [MXScenarioState.RUNNING, MXScenarioState.EXECUTING]:
                            ...
                            MXTEST_LOG_DEBUG(
                                f'Scenario {scenario.name} is not in RUN state...', MXTestLogLevel.WARN)
                else:
                    MXTEST_LOG_DEBUG(
                        f'[SCENARIO RUN CHECK] Scenario {scenario.name} is not in scenario list of {middleware.name}...', MXTestLogLevel.WARN)

            MXTEST_LOG_DEBUG(
                f'Scenario Run Check Success! middleware: {middleware.name}', MXTestLogLevel.PASS)
    ...
    def verify_scenario(self, scenario: MXScenarioElement, timeout: float = 5):
        ...
        if check_result_payload(payload) == None:
            MXTEST_LOG_DEBUG(
                f'{MXElementType.SCENARIO.value} {scenario.name} {MXElementActionType.SCENARIO_VERIFY.value} failed...', MXTestLogLevel.FAIL)
        return self
    ...
    def add_scenario(self, scenario: MXScenarioElement, timeout: float = 5, check_interval: float = 1.0):
        ...
        if check_result_payload(payload) == None:
            MXTEST_LOG_DEBUG(
                f'{MXElementType.SCENARIO.value} {scenario.name} {MXElementActionType.SCENARIO_ADD.value} failed...', MXTestLogLevel.FAIL)
            MXTEST_LOG_DEBUG(
                f'==== Fault Scenario ====', MXTestLogLevel.FAIL)
            MXTEST_LOG_DEBUG(
                f'name: {scenario.name}', MXTestLogLevel.FAIL)
            MXTEST_LOG_DEBUG(
    ...
    def run_scenario(self, scenario: MXScenarioElement, timeout: float = 5):
        ...
        if not scenario.state:
            MXTEST_LOG_DEBUG(
                f'Scenario {scenario.name} state is None... Check it to TIMEOUT. Skip scenario run...', MXTestLogLevel.WARN)
            scenario.schedule_timeout = True
            return False
        if not scenario.schedule_success:
            MXTEST_LOG_DEBUG(
                f'Scenario {scenario.name} is not initialized. Skip scenario run...', MXTestLogLevel.WARN)
            return False
        if not scenario.service_check:
            MXTEST_LOG_DEBUG(
                f'Scenario {scenario.name} is not service validated. Skip scenario run...', MXTestLogLevel.FAIL)
            scenario.schedule_timeout = True
            return False
        ...
        if check_result_payload(payload) == None:
            MXTEST_LOG_DEBUG(
                f'{MXElementType.SCENARIO.value} {scenario.name} {MXElementActionType.SCENARIO_RUN.value} failed...', MXTestLogLevel.FAIL)
        return self
    ...
    def stop_scenario(self, scenario: MXScenarioElement, timeout: float = 5):
        ...
        if check_result_payload(payload) == None:
            MXTEST_LOG_DEBUG(
                f'{MXElementType.SCENARIO.value} {scenario.name} {MXElementActionType.SCENARIO_STOP.value} failed...', MXTestLogLevel.FAIL)
        return self
    ...
    def update_scenario(self, scenario: MXScenarioElement, timeout: float = 5):
        ...
        if check_result_payload(payload) == None:
            MXTEST_LOG_DEBUG(
                f'{MXElementType.SCENARIO.value} {scenario.name} {MXElementActionType.SCENARIO_UPDATE.value} failed...', MXTestLogLevel.FAIL)
        return self
    ...
    def get_whole_scenario_info(self, middleware: MXMiddlewareElement, timeout: float) -> Union[List[MXScenarioInfo], bool]:
        ...
        _, payload, _ = self.publish_and_expect(
            middleware,
            encode_MQTT_message(
                MXProtocolType.WebClient.EM_REFRESH.value % f'{mqtt_client.get_client_id()}_get_whole_scenario_info@{middleware.name}', '{}'),
        ...
        if payload is not None:
            ...
            MXTEST_LOG_DEBUG(
                f'Get whole scenario info of {middleware.name} failed -> MQTT timeout...', MXTestLogLevel.FAIL)
            return False
    ...
    def get_whole_service_list_info(self, middleware: MXMiddlewareElement, timeout: float) -> List[dict]:
        ...
        _, payload, _ = self.publish_and_expect(
            middleware,
            encode_MQTT_message(
                MXProtocolType.WebClient.EM_REFRESH.value % f'{mqtt_client.get_client_id()}_get_whole_service_list_info@{middleware.name}', '{}'),
        ...
        if payload is not None:
            ...
            MXTEST_LOG_DEBUG(
                f'Get whole service list info of {middleware.name} failed -> MQTT timeout...', MXTestLogLevel.FAIL)
            return False
    ...
    def kill_all_middleware(self):
        MXTEST_LOG_DEBUG(f'Kill all middleware...',
                         MXTestLogLevel.INFO, 'red')
        for middleware in self.middleware_list:
            ssh_client = self.find_ssh_client(middleware)
    ...
    def kill_all_thing(self):
        MXTEST_LOG_DEBUG(f'Kill all python instance...',
                         MXTestLogLevel.INFO, 'red')
    ...
    def kill_all_ssh_client(self):
        MXTEST_LOG_DEBUG(f'Kill all ssh client...',
                         MXTestLogLevel.INFO, 'red')
        for ssh_client in self.ssh_client_list:
    ...
    def kill_all_mqtt_client(self):
        MXTEST_LOG_DEBUG(f'Kill all mqtt client...',
                         MXTestLogLevel.INFO, 'red')
        for mqtt_client in self.mqtt_client_list:
            mqtt_client.stop()
            del mqtt_client
    ...
    def kill_all_simulation_instance(self):
        MXTEST_LOG_DEBUG(f'Kill simulation instance...',
                         MXTestLogLevel.INFO, 'red')
    ...
    def publish_and_expect(self, element: MXElement, trigger_msg: mqtt.MQTTMessage = None, target_topic: str = None, auto_subscribe: bool = True, auto_unsubscribe: bool = False, timeout: int = 5):
        if isinstance(element, MXMiddlewareElement):
            target_middleware = element
        else:
            target_middleware = self.find_element_middleware(element)
    ...
    def on_recv_message(self, msg: mqtt.MQTTMessage):
        ...
        if topic_type == MXProtocolType.WebClient.ME_RESULT_SCENARIO_LIST:
            ...
            for event in list(reversed(self.event_log)):
                if event.middleware_element == middleware and event.thing_element == thing and event.event_type == MXEventType.THING_REGISTER:
                    ...
                    MXTEST_LOG_DEBUG(
                        f'[REGISTER] thing: {thing_name} duration: {event.duration:0.4f}', MXTestLogLevel.INFO, progress=progress, color=color)
                    break
            ...
            thing.recv_queue.put(msg)
            self.event_log.append(MXEvent(
                event_type=MXEventType.THING_UNREGISTER, middleware_element=middleware, thing_element=thing, timestamp=timestamp, duration=0))
            ...
            for event in list(reversed(self.event_log)):
                if event.middleware_element == middleware and event.thing_element == thing and event.event_type == MXEventType.THING_UNREGISTER:
                    ...
                    event.error = error_type
                    MXTEST_LOG_DEBUG(
                        f'[UNREGISTER] thing: {thing_name} duration: {event.duration:0.4f}', MXTestLogLevel.INFO)
                    break
            ...
            for event in list(reversed(self.event_log)):
                if event.middleware_element == middleware and event.thing_element == thing and event.service_element == service and event.scenario_element == scenario and event.requester_middleware_name == requester_middleware_name and event.event_type in [MXEventType.FUNCTION_EXECUTE, MXEventType.SUB_FUNCTION_EXECUTE]:
                    event.duration = timestamp - event.timestamp
                    event.error = error_type
                    ...
                    if event.event_type == MXEventType.SUB_FUNCTION_EXECUTE:
                        ...
                        MXTEST_LOG_DEBUG(
                            f'[EXECUTE_SUB] thing: {thing_name} function: {function_name} scenario: {scenario_name} requester_middleware_name: {requester_middleware_name} duration: {event.duration:0.4f} return value: {return_value} - {return_type.value} error: {event.error.value}', MXTestLogLevel.PASS, progress=progress, color=color)
                    elif event.event_type == MXEventType.FUNCTION_EXECUTE:
                        color = 'red' if event.error == MXErrorCode.FAIL else 'green'
                        MXTEST_LOG_DEBUG(
                            f'[EXECUTE] thing: {thing_name} function: {function_name} scenario: {scenario_name} requester_middleware_name: {requester_middleware_name} duration: {event.duration:0.4f} return value: {return_value} - {return_type.value} error: {event.error.value}', MXTestLogLevel.PASS, progress=progress, color=color)
                    break
            ...
            self.event_log.append(MXEvent(
                event_type=MXEventType.SCENARIO_VERIFY, middleware_element=middleware, scenario_element=scenario, timestamp=timestamp, duration=0))
            ...
            for event in list(reversed(self.event_log)):
                if event.middleware_element == middleware and event.scenario_element == scenario and event.event_type == MXEventType.SCENARIO_VERIFY:
                    ...
                    event.error = error_type
                    MXTEST_LOG_DEBUG(
                        f'[SCENE_VERIFY] scenario: {scenario_name} duration: {event.duration:0.4f}', MXTestLogLevel.INFO)
                    break
            ...
            self.event_log.append(MXEvent(
                event_type=MXEventType.SCENARIO_ADD, middleware_element=middleware, scenario_element=scenario, timestamp=timestamp, duration=0))
            ...
            for event in list(reversed(self.event_log)):
                if event.middleware_element == middleware and event.scenario_element == scenario and event.event_type == MXEventType.SCENARIO_ADD:
                    ...
                    color = 'red' if event.error == MXErrorCode.FAIL else 'green'
                    MXTEST_LOG_DEBUG(
                        f'[SCENE_ADD] scenario: {scenario_name} duration: {event.duration:0.4f}', MXTestLogLevel.INFO, progress=progress, color=color)
                    break
            ...
            self.event_log.append(MXEvent(
                event_type=MXEventType.SCENARIO_RUN, middleware_element=middleware, scenario_element=scenario, timestamp=timestamp, duration=0))
            ...
            for event in list(reversed(self.event_log)):
                if event.middleware_element == middleware and event.scenario_element == scenario and event.event_type == MXEventType.SCENARIO_RUN:
                    ...
                    event.error = error_type
                    MXTEST_LOG_DEBUG(
                        f'[SCENE_RUN] scenario: {scenario_name} duration: {event.duration:0.4f}', MXTestLogLevel.INFO)
                    break
            ...
            self.event_log.append(MXEvent(
                event_type=MXEventType.SCENARIO_STOP, middleware_element=middleware, scenario_element=scenario, timestamp=timestamp, duration=0))
            ...
            for event in list(reversed(self.event_log)):
                if event.middleware_element == middleware and event.scenario_element == scenario and event.event_type == MXEventType.SCENARIO_STOP:
                    ...
                    event.error = error_type
                    MXTEST_LOG_DEBUG(
                        f'[SCENE_STOP] scenario: {scenario_name} duration: {event.duration:0.4f}', MXTestLogLevel.INFO)
                    break
            ...
            self.event_log.append(MXEvent(
                event_type=MXEventType.SCENARIO_UPDATE, middleware_element=middleware, scenario_element=scenario, timestamp=timestamp, duration=0))
            ...
            for event in list(reversed(self.event_log)):
                if event.middleware_element == middleware and event.scenario_element == scenario and event.event_type == MXEventType.SCENARIO_UPDATE:
                    ...
                    event.error = error_type
                    MXTEST_LOG_DEBUG(
                        f'[SCENE_UPDATE] scenario: {scenario_name} duration: {event.duration:0.4f}', MXTestLogLevel.INFO)
                    break
            ...
            self.event_log.append(MXEvent(
                event_type=MXEventType.SCENARIO_DELETE, middleware_element=middleware, scenario_element=scenario, timestamp=timestamp, duration=0))
            ...
            for event in list(reversed(self.event_log)):
                if event.middleware_element == middleware and event.scenario_element == scenario and event.event_type == MXEventType.SCENARIO_DELETE:
                    ...
                    event.error = error_type
                    MXTEST_LOG_DEBUG(
                        f'[SCENE_DELETE] scenario: {scenario_name} duration: {event.duration:0.4f}', MXTestLogLevel.INFO)
                    break
        ...
        ####################################################################################################################################################
            ...
            self.event_log.append(MXEvent(
                event_type=MXEventType.SUPER_SCHEDULE, middleware_element=middleware, thing_element=super_thing, service_element=super_service, scenario_element=scenario, timestamp=timestamp, duration=0))
            MXTEST_LOG_DEBUG(
                f'[SUPER_SCHEDULE_START] super_middleware: {super_middleware_name} requester_middleware: {requester_middleware_name} super_thing: {super_thing_name} super_function: {super_function_name} scenario: {scenario_name}', MXTestLogLevel.INFO)
            ...
            for event in list(reversed(self.event_log)):
                if event.middleware_element == middleware and event.thing_element == super_thing and event.service_element == super_service and event.scenario_element == scenario and event.event_type == MXEventType.SUPER_SCHEDULE:
                    event.duration = timestamp - event.timestamp
                    event.error = error_type
                    event.return_type = return_type
                    event.return_value = return_value
                    ...
                    MXTEST_LOG_DEBUG(
                        f'[SUPER_SCHEDULE_END] super_middleware: {super_middleware_name} requester_middleware: {requester_middleware_name} super_thing: {super_thing_name} super_function: {super_function_name} scenario: {scenario_name} duration: {event.duration:0.4f} result: {event.error.value}', MXTestLogLevel.INFO, progress=progress)
                    break
            ...
            self.event_log.append(MXEvent(
                event_type=MXEventType.SUPER_FUNCTION_EXECUTE, middleware_element=middleware, thing_element=super_thing, service_element=super_service, scenario_element=scenario, timestamp=timestamp, duration=0))
            passed_time = get_current_time() - self.simulation_start_time
            progress = passed_time / self.running_time
            MXTEST_LOG_DEBUG(
                f'[SUPER_EXECUTE_START] super_middleware: {super_middleware_name} requester_middleware: {requester_middleware_name} super_thing: {super_thing_name} super_function: {super_function_name} scenario: {scenario_name}', MXTestLogLevel.INFO, progress=progress)
            ...
            for event in list(reversed(self.event_log)):
                if event.middleware_element == middleware and event.thing_element == super_thing and event.service_element == super_service and event.scenario_element == scenario and event.event_type == MXEventType.SUPER_FUNCTION_EXECUTE:
                    event.duration = timestamp - event.timestamp
                    event.error = error_type
                    event.return_type = return_type
                    event.return_value = return_value

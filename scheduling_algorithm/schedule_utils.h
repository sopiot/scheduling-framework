...
cap_result ScheduleUtils_AssignExecutionResult(
    IN cap_handle hVariableHash, IN cap_handle hOutputList,
    IN SExecutionResult *pstExecutionResult);
cap_result ScheduleUtils_ApplySuperScheduleResult(
    cap_handle hScheduleTable, cap_string strScenarioName,
    SScheduleResult *pstScheduleResult);
cap_result ScheduleUtils_IntializeScenario(cap_handle hScheduleTable,
                                           cap_string strScenarioName,
                                           MySchedulingPolicies *pstContext);
cap_result ScheduleUtils_CheckScenarioScheduled(cap_handle hScheduleTable,
                                                cap_string strScenarioName,
                                                cap_bool *pbAllScheduled);
cap_result ScheduleUtils_GetAffectedScenarioOnUnreg(
    MySchedulingPolicies *pstContext, cap_string strThingName,
    cap_handle *phScenarioNameList);
cap_result ScheduleUtils_GetAffectedScenarioOnReg(
    MySchedulingPolicies *pstContext, cap_string strThingName,
    cap_handle *phScenarioNameList);
cap_result ScheduleUtils_ScheduleScenario(MySchedulingPolicies *pstContext,
                                          cap_string strScenarioName);
cap_result ScheduleUtils_UpdateScenario(MySchedulingPolicies *pstContext,
                                        cap_string strScenarioName,
                                        cap_bool *pbScheduled);

cap_result ScheduleUtils_CheckSchedulability(
    IN cap_handle hScheduleTable, IN cap_handle hScenarioTable,
...
cap_result ScheduleUtils_GetWaitingScenarioList(cap_handle hWaitQueue,
                                                cap_string strTargetThingName,
                                                cap_string strTargetServiceName,
                                                cap_handle *phScenarioList);
